...
// ✅ 新增：樣式常數
const HOVER_BLUE = '#1e90ff'; // 滑鼠移入用的藍色
const DEFAULT_STROKE = '#ffaa00ff'; // 一般邊框色
// 顏色定義
...
export default function MapSection() {
  ...
  function applyLayerColors() {
    ...
    gridLayerRef.current.eachLayer((layer: any) => {
      ...
      const temp = toMonthTemp(f, month);
      ...
      const isSelected = selectedCellId && getFeatureId(f) === selectedCellId;
      ...
      (layer as any).setStyle({
        ...
        pane: isSelected ? 'selectedPane' : 'overlayPane', // ✅ 黑框放到最上層 pane
      });
  ...
  useEffect(() => {
    ...
    const raf = requestAnimationFrame(() => {
      ...
      const TPE_BOUNDS = L.latLngBounds(
        [24.50, 121.15], // 南西角 (lat, lng)
        [25.40, 122.35]  // 北東角
      );

      const map = L.map(el, {
        center: [25.0375, 121.565], zoom: 12, minzoom: 11, maxBounds: TPE_BOUNDS.pad(0.02), // 可平移範圍（pad 讓邊界稍微寬一點點）
        ...
        .then((geojson: FeatureCollection) => {
          ...
          const gridLayer = L.geoJSON(geojson as any, {
            ...
            onEachFeature: (feature: any, layer: any) => {
              ...
              layer.on('mouseover', () => {
                const f = layer.feature as GridFeature;
                if (getFeatureId(f) === selectedCellId) {
                  applyLayerColors();
                }

                // const id = getFeatureId(feature);
                // if (selectedCellId && id === selectedCellId) return;

                // // 臨時上藍色（只改視覺，不動 state）
                // (layer as any).setStyle({
                //   color: HOVER_BLUE,     // 邊框藍
                //   weight: 2,
                //   fillOpacity: 0.8,      // 微亮，便於辨識
                // });
          ...
          applyLayerColors(); // ✅ 新增：地圖載好後先套一次樣式
          try {
    ...
    <section ref={sectionRef} className="min-h-screen py-12 px-6 bg-gray-900 relative">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="relative bg-black/50 backdrop-blur-sm rounded-3xl border border-gray-800 p-8 overflow-hidden" style={{ marginTop: '2rem' }}>
          ...
          {/* 地圖容器 */}
          <div id="leaflet-map" ref={mapRef} className="w-full h-[420px] mt-[80px] rounded-2xl overflow-hidden border border-gray-800" />
              ...
              {currentFeature && mode === 'time' && (
                <div id="timeContent">
                  ...
                  <div className="result-display">
                    ...
                    <div className="temp">🌡️ 預測溫度：<span>{timePredicted ?? '--'}</span> °C</div>
